...
from eqopt.models import SolidSolutionModel, CorrectedGibbsModel, PycalphadGibbsModel
...
def optimize():
    ...
    for phase in ['FCC_A1', 'BCT_A5']:
        reference_model=PycalphadReferenceModel(
            ref_db, phase, dtype=torch.float64,
        )
        correction_model=SolidSolutionModel(
    ...
    reference_model = PycalphadReferenceModel(
                ref_db, 'LIQUID', dtype=torch.float64,
            )
    ...
    losses = optimize_thermodynamic_parameters(
        phases, data, steps=40, lr=100.0, print_every=10, loss_threshold=1e-3, 
        stable_weight=1.0,
        unstable_weight=1.0,
